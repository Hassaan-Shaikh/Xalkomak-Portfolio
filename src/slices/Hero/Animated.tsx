...
export default function Animated({ slice }: { slice: Content.HeroSlice }) {
  ...
  return (
    ...
    >
      <div className="hero__heading flex flex-col items-center justify-center text-balance text-center text-6xl font-medium opacity-0 md:flex-row md:text-7xl">
        <div className="hero__svg">
          <PiHandWavingFill className="mb-4 mr-4 md:mb-0" />
        </div>
        <PrismicRichText field={slice.primary.heading} />
      </div>
      <div className="hero__body mx-auto mt-4 text-balance text-center text-2xl text-white opacity-0">
        <PrismicRichText field={slice.primary.body} />
      </div>
      <div className="hero__image glass-container-pfp mt-8 w-fit opacity-0">
        <div className="hero__border absolute inset-0 -z-10 rounded-full bg-[#fff]/75 opacity-0 blur-2xl filter" />
        <PrismicNextImage
